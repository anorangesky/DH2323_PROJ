...
vec3 DirectLight( const Intersection& i ); //task 5
void UpdateMatrix(mat3& matrix, float angle);

int main( int argc, char* argv[] )
{
	screen = InitializeSDL( SCREEN_WIDTH, SCREEN_HEIGHT );
	t = SDL_GetTicks();	// Set start value for timer.
	//task1: to fill the vector with some triangles repr. a 3D model: 
...
void Draw3(){
	...
	// 1. loop through all pixels and compute the corresponding ray direction 
	for(int y = 0; y< SCREEN_HEIGHT; ++y){
		for(int x = 0; x<SCREEN_WIDTH; ++x){
			
			//ray direction vector:
			vec3 d(x-(SCREEN_WIDTH/2) , y-(SCREEN_HEIGHT/2) , focalLength);
			// 2. Call closestIntersection to get the closest intersesction in that dir
			hit = ClosestIntersection(cameraPos *R, d, triangles, closestInt);
			if(hit){
			// 3. If true -> set the color of the pixel to 
			// 		the color of the intersected triangle
...
vec3 DirectLight( const Intersection& i ){
	...
	//	1. calculate the unit vector describing the direction from the surface to the light source
		...
		if(a.triangleIndex != i.triangleIndex && a.distance < rDist){
			//if(a.triangleIndex != i.triangleIndex && a.distance < rDist){
			//surface is in shadow if the distance to closest intersecting surface 
			// is closer than the light source
			return vec3(0,0,0);
			//}
		}
			...
			//	3. compute the direction using "D = (P * max(r dot n, 0))/4*pi*r²" and return it
...
bool ClosestIntersection(vec3 start,vec3 dir,const vector<Triangle>& triangles,Intersection& closestIntersection){
	...
	//Task 2.3: intersection computations:
		...
		if((u>=0)&&(v>=0)&&((u+v)<=1)&&(t > 0)){
			...
			vec3 r2 = start + (t*dir);
			float distance = glm::distance(r2, start); //guess i dont need this because t is already this. 
